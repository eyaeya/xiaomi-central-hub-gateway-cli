...
| 可达性（never-fires sink） | `rule lint --strict`（读时报）；`rule enable` 写时硬拦 | `卡片不可达`：动作卡（`deviceOutput`/`varSetNumber`/`varSetString`/`deviceGetSetVar`）所在连通分量里没有任何触发卡（`onLoad`/`alarmClock`/`deviceInput`/`varChange`/`timeRange`/`loop`/`register`/`deviceInputSetVar`），规则永不触发该动作（网页画布因为肉眼可见这条断链而藏住了这个坑，CLI 的「先保存后启用」会让一条永不触发的规则被静默启用） | **仅 `rule enable`**（抛 `ConfigError` 退出 5）；`rule set`/`setGraph` 不跑（增量编写允许卡片悬空待连线）；`rule validate` 也不报，要用 `rule lint --strict` 提前看到 |
...
| `卡片不可达: <类型> sink ... has no upstream trigger`（enable exit 5 / lint error） | 动作卡所在子图里没有任何触发卡，规则永不触发该动作 | 用 `rule edge add` 把触发器（事件源 / `onLoad` / `alarmClock` 等）连到该动作的上游；连完 `rule lint --strict` 复查 |
