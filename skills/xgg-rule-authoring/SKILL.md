...
| 有向可达性（never-fires sink） | `rule lint --strict`（读时报）；`rule enable` 写时硬拦 | `卡片不可达`：没有独立事件源沿有效源→目标边到达动作卡（`deviceOutput`/`varSetNumber`/`varSetString`/`deviceGetSetVar`）。独立来源只有 `onLoad`/`alarmClock`/`deviceInput`/`deviceInputSetVar`/`varChange`；`timeRange` 是条件状态，`loop` 需上游驱动 `start`，`register` 需上游驱动 `setTrue`/`setFalse`，三者都不是 bootstrap source | **仅 `rule enable`**（抛 `ConfigError` 退出 5）；`rule set`/`setGraph` 不跑（增量编写允许卡片悬空待连线）；`rule validate` 也不报，要用 `rule lint --strict` 提前看到 |
...
| `卡片不可达: <类型> sink ... has no upstream independent event source`（enable exit 5 / lint error） | 没有独立事件源沿有效有向边抵达动作卡；下游或无控制输入的 `loop`/`register`、单独的 `timeRange` 不能反向证明可达 | 用 `rule edge add` 把 `onLoad`/`alarmClock`/`deviceInput`/`deviceInputSetVar`/`varChange` 之一连到动作上游；`loop` 接 `start`，`register` 接 `setTrue`/`setFalse`，连完 `rule lint --strict` 复查 |
