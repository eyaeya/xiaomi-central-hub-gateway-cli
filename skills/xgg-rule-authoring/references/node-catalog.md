...
选择卡片、确认每类 flag/default/pin，或审查 raw JSON 时读取本文。本表记录当前 xgg typed model、schema、validator 与测试共同维护的 canonical 节点契约；目标设备参数另见 [device-semantics.md](device-semantics.md)。

## 目录

- [禁止的非 canonical 节点写法](#禁止的非-canonical-节点写法)
...
无论片段来自旧导出、第三方文档还是手写猜测，只要采用下列形状，就不属于当前 xgg 的 canonical 节点契约：

| 非 canonical 写法 | 当前 canonical wire |
|---|---|
...
Agent 不得把历史或第三方 JSON “微调后试写”。先用 typed shortcut；raw 必须来自当前 `rule view`/export 或本文 canonical schema，并先通过当前 xgg 校验。
...
| `deviceOutput` | `trigger:E` | `output:E` | 可串接；静态契约不证明 success 后才发 |
...
省略 `--id` 时生成 `n` + 32 位小写十六进制 UUID（去掉连字符）；当前 xgg 的新建 typed node ID 契约要求显式 ID 是非空 ASCII alphanumeric `[A-Za-z0-9]+`，不能含 `- _ . :`、空白或 Unicode。为后续 wire 明确传稳定 ID。既有图中的非 canonical ID 要保真观察/导出并报告，不能静默改名，因为所有 source outputs 都引用它。
...
既有 event-mode `deviceInput` 的整数 `cfg.version:0` 仍可读取/校验；当前 xgg typed export/import 重放会 canonicalize 为 `version:1`，不要把 `0` 继续复制到新卡。property mode 新卡同样生成 `version:1`。
...
`preload` 只用于 `deviceInput` / `deviceInputSetVar` property 与 `varChange`，默认 false；true 表示 enable 时先查询/评估一次，后续 notify/change 路径不变。它不能制造 notify/read/push。当前 `onLoad` 节点契约只证明它是无输入 event source；此前目标网关安全实测确认 disable→enable 会触发，save/restart/restore 等时机仍未证明。
...
typed literal suffix 可用 canonical `ms|s|min|hour` 或别名 `m|h`，新 cfg 只写 canonical unit；runtime 永远是换算后的毫秒，二者必须一致。既有 raw `cfg.unit:"m"` 仅为历史兼容可读/重放，`"h"` 不是合法 raw unit。当前 xgg schema/typed validator 对 `delay.timeout` 与 `loop.interval` 只要求整数，允许 0/负数；`statusLast.timeout` 与 `eventSequence.timeout` 则必须大于 0。前者是可表达/可保存能力，不等于固件已证明非正值的 executor 行为。disabled 规则只能证明保存；真正研究必须获明确授权，使用隔离临时规则、无物理副作用 marker、外部超时/硬停止、日志与回滚，尤其非正 `loop` 要防事件风暴。普通规则仍用正值。
