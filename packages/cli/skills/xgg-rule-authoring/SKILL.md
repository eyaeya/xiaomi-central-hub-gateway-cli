...
| 按 pin 聚合的有向可达性（never-fires sink） | `rule lint --strict`（读时报）；`rule enable` 写时硬拦 | `卡片不可达`：独立事件源虽存在，但无法按目标卡的必需输入语义驱动动作卡（`deviceOutput`/`varSetNumber`/`varSetString`/`deviceGetSetVar`）。`eventSequence` 要全部事件输入；`condition.unmet` 要 trigger，`condition.met` 还要状态；`logicAnd` 要全部状态，直接向事件下游传播时还要至少一路更新（纯状态组合可再经 `statusLast` 桥接）；`logicOr`/`signalOr` 任一路即可。`timeRange` 只供状态，不能直接 bootstrap，但可经 `statusLast` 桥接成事件；`loop.stop`/`onlyNTimes.zero` 也不能单独向下游传播 | **仅 `rule enable`**（抛 `ConfigError` 退出 5）；`rule set`/`setGraph` 不跑（增量编写允许卡片悬空待连线）；`rule validate` 也不报，要用 `rule lint --strict` 提前看到 |
...
> **空网关也能从零构造,不需要任何现成规则可抄。** 最稳的冷启动是用 shortcut(第五、十节):一条 `rule node add --type <T> <flags>` 不写一行 JSON 就能产出合法卡片,缺什么参数 `--help` 会告诉你。只有「某卡片没有 shortcut」「要复刻网页里的复杂卡片」「想一次原子推送整张图」时才需要手写 JSON——这时**不要猜结构,直接抄附录 A**(整图信封 + 25 类节点精确 JSON + 比较算子 wire 编码,均经离线 `rule validate --body` 实测零错零警)。有现成规则时 `xgg rule view <id>` 拿到的就是可改可写回的整图 JSON;空网关没得 view,就以附录 A 的模板起步。
...
| `卡片不可达: <类型> sink ... has no satisfiable upstream event path`（enable exit 5 / lint error） | 独立事件源没有按目标 pin 的聚合规则抵达动作卡；常见是 `eventSequence` 只活一路、`logicAnd` 缺状态或直接事件路径只有静态状态、`condition` 的目标分支缺 trigger/状态、或只接了 `loop.stop`/`onlyNTimes.zero` | 补齐对应必需输入：`eventSequence` 全部事件、`logicAnd` 补全状态并为直接事件路径提供至少一路更新（纯状态组合要触发可接 `statusLast`）、`condition.unmet` 补 trigger、`condition.met` 再补状态；控制卡改接能驱动输出的输入，连完 `rule lint --strict` 复查 |
